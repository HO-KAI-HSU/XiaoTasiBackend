...
namespace travelManagement.Controllers
{
    public class GroupController : Controller
    {
        ...
        public ActionResult GetGroupList()
        {
            ...
            // SQL Command
            SqlCommand select = new SqlCommand("select group_id, name from group_list", connection);
            // 開啟資料庫連線
            connection.Open();
            ...
            while (reader.Read())
            {
                ...
                groupData.name = reader.IsDBNull(1) ? "" : (string)reader[1];
        ...
        public ActionResult GetGroupInfo(int groupId)
        {
            SqlConnection connection = new SqlConnection(this.sql_DB);
            // SQL Command
            SqlCommand select = new SqlCommand("select group_id, name from group_list WHERE group_id = @groupId", connection);
